...
<x-ui.sheet name="detail-reservation" title="Reservation Details" maxWidth="md">
    @if($reservation)
        @php
        ...
        @if($reservation['internal_notes'])
            <div class="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 mb-4">
    ...
    <x-slot:footer>
        @if($reservation)
            <x-ui.button variant="secondary" @click="$dispatch('close-sheet', 'detail-reservation')">
                Close
            </x-ui.button>
            <x-ui.button wire:click="openEditSheet({{ $reservation['id'] }})">
                Edit Reservation
            </x-ui.button>
        @endif
    </x-slot:footer>
