...
@elseif($isPassword)
    @php $toggleId = 'pw_toggle_' . $inputId; @endphp
    <div class="relative">
        <input
            type="password"
            id="{{ $inputId }}"
        ...
        <button
            type="button"
            id="{{ $toggleId }}"
            onclick="(function(btn){var inp=document.getElementById('{{ $inputId }}');var showing=inp.type==='text';inp.type=showing?'password':'text';btn.querySelector('.pw-show').style.display=showing?'':'none';btn.querySelector('.pw-hide').style.display=showing?'none':''})(this)"
            ...
            tabindex="-1"
        >
            <svg class="pw-show w-5 h-5" style="display:none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            </svg>
            <svg class="pw-hide w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
