<div
    ...
    x-data="combinedOrderCart({{ json_encode($this->initialCart) }})"
    x-init="init()"
>
    <x-ordermanagement.styles />

    {{-- ══════════════════════════════════════════════════════════
         Table Selector (shown when no table selected)
    ══════════════════════════════════════════════════════════ --}}
    @if(!$table)
        <div class="flex items-center justify-center h-screen p-4">
            <div class="bg-white rounded-2xl shadow-xl border border-gray-200 p-8 max-w-md w-full">
                <div class="text-center mb-6">
                    <div class="w-16 h-16 bg-molveno-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" class="text-molveno-blue-600">
                            <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>
                            <line x1="3" y1="9" x2="21" y2="9"/>
                            <line x1="9" y1="21" x2="9" y2="9"/>
                        </svg>
                    </div>
                    <h1 class="text-xl font-bold text-gray-900">Select a Table</h1>
                    <p class="text-sm text-gray-500 mt-1">Choose the table you want to place an order for.</p>
                </div>
                ...
                <div class="space-y-2 max-h-80 overflow-y-auto">
                    @foreach($this->availableTables as $tbl)
                        <button
                            ...
                            class="w-full flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:border-molveno-blue-300 hover:bg-molveno-blue-50 transition-all text-left"
                        >
                            <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-500 font-semibold">
                                {{ substr($tbl->table_name, 0, 2) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-gray-900">{{ $tbl->table_name }}</p>
                                <p class="text-xs text-gray-500">{{ $tbl->floorPlan?->name ?? 'Main Hall' }}</p>
                            </div>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" class="text-gray-400">
                ...
                <div class="mt-6 pt-4 border-t border-gray-100 text-center">
                    <a href="{{ route('tablemanagement') }}" class="text-sm text-gray-500 hover:text-molveno-blue-600 flex items-center justify-center gap-1">
    ...
    @else
    {{-- ══════════════════════════════════════════════════════════
         Layout: sidebar + main (matches order-page styling)
    ══════════════════════════════════════════════════════════ --}}
        ...
        >
            ...
            <div class="flex-1 overflow-hidden">
                <livewire:orders.sidebar
                    ...
                    :targetComponent="\App\Livewire\Orders\CombinedOrderPage::class"
                    wire:key="combined-sidebar-{{ $table->id }}"
                />
            </div>
        </aside>
        ...
        >
            ...
            <div class="flex-1 overflow-hidden">
                <livewire:orders.sidebar
                    ...
                    :floorPlanId="$table->floor_plan_id"
                    :targetComponent="\App\Livewire\Orders\CombinedOrderPage::class"
                    wire:key="combined-sidebar-mobile-{{ $table->id }}"
                />
            </div>
        </div>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto pb-24">
                <div class="max-w-screen-xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-5">
                    ...
                    {{-- Page header --}}
                    <x-ui.page-header
                        title="New Combined Order — {{ $table->table_name }}"
                    ...
                    >
                        <x-slot:actions>
                            ...
                            <x-ui.button variant="secondary" wire:click="switchTable">
                            ...
                            @foreach($this->dishes as $dish)
                                ...
                                >
                                    ...
                                    <div class="dish-card-body">
                                        {{-- Name + dietary icons + routing tag --}}
                                        <div class="flex items-start gap-2 flex-wrap">
                                            ...
                                            <div class="flex items-center gap-1 mt-0.5">
                                                ...
                                                @endif
                                                @if($dish->routing_tag)
                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-semibold {{ $dish->routing_tag === 'bar' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700' }}">
                                                        {{ $dish->routing_tag === 'bar' ? 'Bar' : 'Kitchen' }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
                    ...
                    @endif
                    @endif {{-- end @if(!$activeMenuId && !$barMode) --}}
    ...
    ══════════════════════════════════════════════════════════ --}}
    <script>
        function combinedOrderCart(initialCart = {}) {
            return {
